	...
	$wp_customize->add_section(
		new Corporate_Key_Upsell_Section( $wp_customize, 'custom_theme_upsell',
			array(
				...
				'pro_url'  => 'http://axlethemes.com/downloads/corporate-key-pro/',
				'priority' => 1,
			)
		)
	);
...
function corporate_key_customizer_control_scripts() {
	...
	wp_enqueue_script( 'corporate-key-customize-controls', get_template_directory_uri() . '/js/customize-controls' . $min . '.js', array( 'jquery', 'customize-controls' ), '1.0.0' );

	wp_enqueue_style( 'corporate-key-customize-controls', get_template_directory_uri() . '/css/customize-controls' . $min . '.css', array(), '1.0.0' );
